...
const DetailPackageCard = ({ route, navigation }) => {
  ...
  const getPriceTotal = () => {
    if(data.food.price && data.drink.price && count !== 0){
      setPriceTotal(count * (data.package.price))
    }
  }
  ...
  const fetchBankData = async () => {
    try {
      ...
      setBankData(data);
      setIsModalVisible(true); // Tampilkan modal setelah mendapatkan data
    } catch (error) {
        ...
        <View className="flex-1 items-center">
          ...
          >
            <View className="flex-row gap-3">
              <View className="shadow bg-white flex justify-center items-center px-5 py-5">
                <Text className="font-bold text-lg mb-1">
                  Makanan
                </Text>
                <Image
                  source={{
                    uri: `data:${data.food.typeImage};base64,${data.food.dataImage}`,
                  }}
                  style={{
                    width: 200,
                    height: 200,
                ...
                <Text className="text-lg mb-4">
                  Harga:
                  {data.food.price.toLocaleString(
                    "id-ID"
                  )}
                </Text>
              </View>
              ...
              <View className="shadow bg-white flex-col justify-center items-center pt-5 px-5 py-5">
                ...
                <Image
                  source={{
                    uri: `data:${data.drink.typeImage};base64,${data.drink.dataImage}`,
                  }}
                  style={{
                    width: 200,
                    height: 200,
                ...
                <Text className="text-lg mb-4">
                  Harga:
                  {data.drink.price.toLocaleString(
                    "id-ID"
                  )}
                </Text>
                </View>
          ...
          {data.package.discount.toLocaleString('id-ID')}
          </Text>
