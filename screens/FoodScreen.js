...
const FoodScreen = ({navigation}) => {
  ...
  useLayoutEffect(() => {
    getTokenCookie();
    if (foodData.length === 0 && token !== "") {
      getAPIPackages()
    }
    navigation.setOptions({
      headerShown: false,
    });
  }, [layer, dataFetched, foodData, token, imageUri, canPostData]);
  ...
  const getTokenCookie = async () => {
    ...
    const postOrderData = async () => {
      ...
      try {
        ...
        const response = await axios.post("https://backend-chatering-online.vercel.app/api/v1/admin/food/create", formData ,{
          headers:{
            "Authorization": `${token}`,
    ...
    return (
      ...
      <View>
        <ShowFood imageUri={imageUri} navigation={navigation} foods={foodData} token={token}/>
      </View>
