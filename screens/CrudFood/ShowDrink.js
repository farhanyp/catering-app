...
const ShowDrink = ({navigation, drinks, token}) => {
  ...
  const [selectedFood, setSelectedDrink] = useState(null);
  ...
  useLayoutEffect(() => {
  ...
  }, [drinks, name, qty, price, description]);

    const deleteCard = async(id)=>{
        try{
            const response = await axios.delete(`https://backend-chatering-online.vercel.app/api/v1/admin/drink/${id}`,{
                headers: {
                    'Authorization': `${token}`,
                  },
            })
    ...
    const updateOrderData = async () => {
      try {
        ...
        const response = await axios.patch(`https://backend-chatering-online.vercel.app/api/v1/admin/drink/${DrinkId}`, formData ,{
          headers:{
            "Authorization": `${token}`,
